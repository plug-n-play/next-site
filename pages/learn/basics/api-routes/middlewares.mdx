...
Now Let's update our page to handle a filter by author. Open `pages/index.js` and replace its content with the following:
...
export default function Index() {
  ...
  const { data, error } = useSWR(
    query.author ? `/api/randomQuote?author=${query.author}` : '/api/randomQuote',
    fetcher
  );
...
Now go to http://localhost:3000/?author=rauch and refresh the page multiple times, what can you notice?
